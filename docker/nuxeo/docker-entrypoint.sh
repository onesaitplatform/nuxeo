...
envsubst '${NUXEO_KEYCLOAK_REALM} ${NUXEO_KEYCLOAK_AUTHSERVERURL} ${NUXEO_KEYCLOAK_RESOURCE} ${NUXEO_KEYCLOAK_PUBLICCLIENT} ${NUXEO_KEYCLOAK_CREDENTIALS_SECRET} ${NUXEO_KEYCLOAK_USERESOURCEROLEMAPPINGS} ${INITIAL_JAVA_OPTS} ${NUXEO_URL} ${NUXEO_DB_NAME} ${NUXEO_DB_USER} ${NUXEO_DB_PASSWORD} ${NUXEO_DB_HOST} ${NUXEO_DB_PORT} ${NUXEO_DB_JDBC_URL} ${NUXEO_DB_DRIVER} ${NUXEO_DB_MIN_POOL_SIZE} ${NUXEO_DB_MAX_POOL_SIZE} ${NUXEO_VCS_MIN_POOL_SIZE} ${NUXEO_VCS_MAX_POOL_SIZE} ${NUXEO_MAIL_TRANSPORT_HOST} ${NUXEO_MAIL_TRANSPORT_PORT} ${NUXEO_MAIL_TRANSPORT_AUTH} ${NUXEO_MAIL_TRANSPORT_USER} ${NUXEO_MAIL_TRANSPORT_PASSWORD} ${NUXEO_MAIL_FROM} ${NUXEO_MAIL_TRANSPORT_PROTOCOL} ${NUXEO_MAIL_TRANSPORT_STARTTLS_ENABLE}' < /etc/nuxeo/nuxeo.conf.template > /etc/nuxeo/nuxeo.conf


${NUXEO_MAIL_TRANSPORT_HOST} ${NUXEO_MAIL_TRANSPORT_PORT} ${NUXEO_MAIL_TRANSPORT_AUTH} ${NUXEO_MAIL_TRANSPORT_USER} ${NUXEO_MAIL_TRANSPORT_PASSWORD} ${NUXEO_MAIL_FROM} ${NUXEO_MAIL_TRANSPORT_PROTOCOL} ${NUXEO_MAIL_TRANSPORT_STARTTLS_ENABLE}
...
if [ ! -f $NUXEO_HOME/configured ]; then
  ...
  if [ -n "$NUXEO_PACKAGES" ]; then
    ...
    installCommand="nuxeoctl mp-install $NUXEO_PACKAGES --accept=true --relax yes"
    if [ "$NUXEO_DEV" = true ]; then
